...
A rotation is visible without reading logs. Eight events go on the secret —
...
consequence rather than the news, so `RotationCompleted` would report a
`drop-old` somebody asked for when what happened is that the rollback the hold
existed for had already become impossible. They are two reasons and not one
because the reason is what a human triages on and a repair throws nothing away.
Both are recorded durably in `spawnery.cloud/ca-rotation-discarded`, which the
next accepted `start` clears; that annotation is the only one in the table
above the operator writes about a slot rather than about the sequence.

`internal/certs/metrics.go` registers two gauges in the shape the other
