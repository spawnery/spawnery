...
`TestSwitchingToTheNextCAPairsEveryCertificateWithItsOwnKey`. It must go red —
not inside `Validate`, but out of the `SwitchToNext` call itself, since
`SwitchToNext` hands the mismatched pair to `Reissue` before any serving
certificate exists, and `Reissue`'s own `parseCA` rejects it there. Restore,
and confirm with `git diff --stat internal/certs`. Record the verbatim
failure in the report; a mutation that leaves the test green means the test
