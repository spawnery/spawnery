...
- **`make e2e` installs somewhere else on purpose** (`ddfcf3a..a10bac4`, one
  ...
  --create-namespace`, and `test/e2e/e2e_test.go:49`'s `operatorNamespace`
  ...
  (`hack/e2e.sh:174-181`), which reads the applied
...
(`internal/rbacaudit/deploy_envtest_test.go:468`), and incidentally rather
...
- **`make manifests` has a second half.** `Makefile:14-19`: controller-gen
  ...
  The script's two guards check that controller-gen's *output* still has the
  shape the `sed` transforms are anchored to — not that either transform
  actually applied. A broken `sed` with an intact input exits 0 and writes a
  corrupted template; §5 below has the ledger entry.
- **`internal/rbacaudit`'s tests require `helm` on `PATH`.** `renderChart`
  (`internal/rbacaudit/deploy_envtest_test.go:161-184`) runs `helm template`
...
- **Three distinct namespaces exist, and each is what it is on purpose.**
  `spawnery-system` is the chart's documented default (never templated as a
  literal anywhere; it only appears in `charts/spawnery/README.md`'s install
  command and in the two `+kubebuilder:rbac` placeholder comments).
  `platform-system` is where `hack/e2e.sh` actually installs the chart for
  `make e2e` (`hack/e2e.sh:31`, `test/e2e/e2e_test.go:49`). `audit-system` is
...
4. **Task 2, minor, deferred — carried forward as still open.**
   `hack/chart-templates.sh`'s two guards check controller-gen's *input*
   shape, not that either `sed` transform actually applied: a broken `sed`
   with an intact input exits 0 and writes a corrupted `rbac.yaml` or
   `crds.yaml`. The reviewer reproduced both bypasses directly. The
   suggested fix is small — a postcondition checking `rbac.yaml` has exactly
   one `{{ .Release.Namespace }}` and no `spawnery-system`, and `crds.yaml`
   has the `keep` annotation four times — and remains unimplemented at the
...
6. **Task 3, fix round 1, two findings addressed.** The re-reviewer ran the
   ...
   it is supposed to.
7. **Task 3, minor, deferred.** `audit_envtest_test.go:301-302`'s own
   comment still claims `readForwardingSecretReader` uses "the same
   multi-document split `readGeneratedRoles` uses" — Task 3 moved
   `readGeneratedRoles` off that shared path onto `renderedManifest`, so the
   comment is now false. The sibling comment in `deploy_envtest_test.go` was
   corrected in the same fix round; this one was missed. Caught by reading.
...
11. **Task 4, minor, deferred.** The forwarding-secret path is invisible to
    ...
    nothing (`test/e2e/e2e_test.go:180-186`), and no scenario reads
...
12. **Task 4, Important, fix round 1 dispatched and closed.** The `sed`
    ...
    `check_forwarding_secret_reader_subject`
    (`hack/e2e.sh:174-181`), which reads the applied object back and exits
...
15. **Task 4, minor, deferred, predates this milestone.**
    `test/e2e/e2e_test.go:157-158` cites `(task-4-report.md, "Fix round
...
No whole-branch review beyond the six task reviews above has run against
this milestone as of this document; that gate, if this project runs one the
way milestone 6c did, is 6e's or the merge process's to schedule.
