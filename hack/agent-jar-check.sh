...
JAR="${1:?usage: agent-jar-check.sh <jar> [source-dir]}"
# Optional: the plugin's source root. Given one, the no-Java constraint below is
# checked too. nix/paper-agent.nix passes it; a hand invocation on a store path
# has no source to point at and says so rather than passing silently.
SRC="${2:-}"
...
if [ -n "$stray" ]; then
	...
	sed -e 's|^|  |' <<<"$stray" >&2
	fail "every class the plugin ships must be under cloud/spawnery/agent/ -- add the package to the relocate list in agent/paper/build.gradle.kts"
fi
...
# The generated stubs reach the jar through shadowJar's explicit from(proto)
# (they are in a source set of their own, so sourceSets.main.output does not
# carry them). Dropping that line produces a jar that installs, passes every
# check above, and cannot construct a single message.
...
descriptor="$(unzip -p "$JAR" paper-plugin.yml)"
grep -q '^version:' <<<"$descriptor" ||
	fail "paper-plugin.yml carries no version"
if grep -q '\${' <<<"$descriptor"; then
	fail "paper-plugin.yml still holds an unexpanded placeholder; processResources did not expand it"
fi

# The plugin is Kotlin, and the generated Java is isolated in a source set whose
# compile classpath never receives Paper's jars (see build.gradle.kts). A .java
# under src/main or src/test would break that in a way that is silent both ways:
# under src/main/java it compiles against the class-file-major-69 Paper jars and
...
if [ -n "$SRC" ]; then
	...
	if [ -n "$strayjava" ]; then
		...
		sed -e 's|^|  |' <<<"$strayjava" >&2
		fail "src/main and src/test hold Kotlin only; generated Java belongs in src/proto/java"
	fi
else
